...
export function ConsentPanel({
  ...
  return (
    <div
      className="my-3 rounded-2xl bg-white shadow-sm border border-purple-200"
    ...
    >
      ...
      <div className="px-5 py-4 border-b border-gray-200">
        ...
        <p className="text-sm text-govuk-dark-grey mt-2 ml-[38px]">
          The following services need to share data on your behalf. Review each
          request and decide whether to allow it.
        </p>
      </div>
...
/** Summary card shown when all consents are decided */
export function ConsentSummaryCard({
  ...
  const allDecided =
    grants.length > 0 && grants.every((g) => decisions[g.id] !== undefined);
  if (!allDecided || submitted) return null;

  const grantedCount = Object.values(decisions).filter(
    (d) => d === "granted",
  ).length;
  ...
  return (
    <div
      className="my-3 rounded-2xl bg-white border border-purple-200 shadow-sm"
      style={{ boxShadow: "0 2px 8px rgba(145,43,136,0.08)" }}
    >
      <div className="px-5 py-5">
        <div className="flex items-center gap-2.5 mb-3">
          ...
          <span className="text-sm font-bold text-purple-700">
            Consent decisions ready
          </span>
        </div>
        <p className="text-sm text-govuk-dark-grey mb-1">
          {grantedCount} agreed, {deniedCount} declined
        </p>
        <button
          onClick={onSubmit}
          className="mt-3 text-sm font-bold text-white px-6 py-2.5 rounded-full transition-colors"
          style={{ backgroundColor: "#912b88" }}
        >
          Confirm consent decisions
        </button>
      </div>
    </div>
  );
