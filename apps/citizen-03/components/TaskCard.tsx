...
function generateCalendarFile(task: {
  id: string;
  description: string;
  detail: string;
  dueDate?: string | null;
...
> = {
  ...
  address: {
    label: "Address",
    type: "text",
    placeholder: "e.g. 12 Maple Road, Manchester",
  },
  ...
  employer_name: {
    label: "Employer name",
    type: "text",
    placeholder: "e.g. TechCo Solutions Ltd",
  },
  employment_status: {
  ...
  landlord_address: {
    ...
    type: "text",
    placeholder: "e.g. 45 Oak Street, Manchester, M1 4BH",
  },
};
...
function matchPeopleInText(
  ...
  const topChildren = personaData.children as
  ...
  if (family) {
    const famChildren = family.children as
      | Array<Record<string, unknown>>
      | undefined;
...
function inferInteraction(
  ...
  const enumMatch =
    text.match(/(?:choose|select|pick|between)\s+(.+?)(?:\.|$)/i) ??
...
function inferFields(description: string, detail: string): string[] {
  ...
  const rules: Array<{ pattern: RegExp; fields: string[] }> = [
    ...
    {
      pattern: /employment.*(end|last)|last\s*day|end\s*date/,
      fields: ["employment_end_date"],
    },
    { pattern: /employer/, fields: ["employer_name"] },
    {
      pattern: /landlord.*(name|full)|name.*landlord/,
      fields: ["landlord_name"],
    },
    {
      pattern: /landlord.*(address|street|complete)/,
    ...
    {
      pattern: /national\s*insurance|ni\s*number/,
...
function getPreFill(
  ...
  const bankAccounts = fin?.bankAccounts as
    | Array<Record<string, unknown>>
    | undefined;
  ...
  switch (fieldKey) {
    case "email":
    case "contact_email":
      return String(pc?.email ?? "");
    case "phone":
    case "phone_number":
    case "contact_phone":
      return String(pc?.phone ?? "");
    case "full_name":
      return pc ? `${pc.firstName ?? ""} ${pc.lastName ?? ""}`.trim() : "";
    case "date_of_birth":
      return String(pc?.dateOfBirth ?? "");
    case "national_insurance_number":
      return String(pc?.nationalInsuranceNumber ?? "");
    case "address":
      return addr
        ? [
            addr.line1 ?? addr.line_1,
            addr.line2 ?? addr.line_2,
            addr.city,
            addr.postcode,
          ]
            .filter(Boolean)
        ...
        : "";
    case "tenure_type":
      return String(addr?.housingStatus ?? "");
    case "employer_name":
      return String(
        empData?.employer ??
          empData?.previousEmployer ??
          empData?.businessName ??
          "",
      );
    case "employment_status":
      return String(empData?.status ?? "");
    case "employment_end_date":
      return String(
        empData?.employmentEndDate ?? empData?.retirementDate ?? "",
      );
    case "sort_code":
      return String(primaryBank?.sortCode ?? "");
    case "account_number":
      return String(primaryBank?.accountNumber ?? "");
    case "monthly_rent":
    case "rent_amount":
      return String(fin?.monthlyRent ?? fin?.monthlyMortgage ?? "");
    case "income_amount":
      return String(empData?.annualIncome ?? "");
    default:
      return "";
  }
}
...
export function TaskCard({
  ...
  const hasLLMFields =
    !isAgent && Array.isArray(task.fields) && task.fields.length > 0;
  const hasOptions =
    !isAgent && !hasLLMFields && task.options && task.options.length > 0;
  const hasExplicitFields =
    !hasOptions &&
    !hasLLMFields &&
    !isAgent &&
    task.dataNeeded &&
    task.dataNeeded.length > 0;
  const [showAgentDetail, setShowAgentDetail] = useState(false);

  const inferredFields =
    !isAgent && !hasOptions && !hasLLMFields && !hasExplicitFields
  ...
  const personaData = useAppStore((s) => s.personaData) as Record<
    string,
    unknown
  > | null;
  ...
  const smartInteraction =
    !isAgent && !hasLLMFields && !hasOptions && !hasActiveFields
      ? inferInteraction(task.description, task.detail, personaData)
      : null;
  const hasSmartOptions =
    smartInteraction !== null && smartInteraction.type !== "freeform";
  ...
  const shadowStyle = isCompleted
    ...
    : isAgent
      ? "0 2px 8px rgba(29,112,184,0.08)"
      : "0 2px 8px rgba(0,112,60,0.08)";
  const iconBg = isCompleted
    ...
    : isAgent
      ? "bg-blue-100"
      : "bg-green-100";
  const iconColor = isCompleted ? "#00703c" : isAgent ? "#1d70b8" : "#00703c";
  const titleColor = isCompleted
    ? "text-green-700"
    : isAgent
      ? "text-blue-700"
      : "text-green-700";
  const badgeLabel = isCompleted ? "Submitted" : isAgent ? "Agent" : "You";
  ...
  const handleAccept = () => {
    if (hasLLMFields) {
      ...
      onComplete?.(
        task.id,
        `Selected: ${chosen.map((o) => o.label).join(", ")}`,
      );
    } else if (hasActiveFields) {
      const parts: string[] = [];
      for (const key of activeFields) {
        if (
          (key === "monthly_rent" || key === "rent_amount") &&
          fieldValues.tenure_type
        ) {
        ...
        if (val) {
          ...
          if (
            ...
            label =
              getRentLabel(fieldValues.tenure_type) ?? "Monthly rent (\u00a3)";
          } else {
      ...
      if (selectedSmart === "other" && otherText.trim()) {
        ...
        const chosen = smartInteraction!.options.find(
          (o) => o.value === selectedSmart,
        );
  ...
  const hasFilledField = hasLLMFields
    ...
    : hasOptions
      ...
      : hasSmartOptions
        ? selectedSmart !== null &&
          (selectedSmart !== "other" || otherText.trim().length > 0)
        : hasActiveFields
          ? Object.values(fieldValues).some((v) => v.trim().length > 0)
          : true;

  const isDateReminder =
    !isAgent &&
    !hasLLMFields &&
    !!task.dueDate &&
    !hasActiveFields &&
  ...
  return (
    <div
      className={`my-3 rounded-2xl bg-white shadow-sm transition-opacity border ${borderClass} ${isCompleted ? "opacity-80" : ""}`}
    ...
    >
      <div className="px-5 py-5">
        ...
        {/* Content */}
        <p
          className={`text-base font-medium ${isCompleted ? "text-govuk-dark-grey" : "text-govuk-black"}`}
        >
          {task.description}
        </p>
        {!isCompleted && (
                ...
                return (
                  <div key={field.key}>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        updateLlmField(field.key, e.target.value)
                      }
                      disabled={disabled}
        ...
        {/* Data needed tags for agent tasks */}
        {!isCompleted &&
          !hasActiveFields &&
          isAgent &&
          task.dataNeeded &&
            ...
            return (
              <div className="mt-3">
                {useTable ? (
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-gray-200">
                        <th className="text-left text-xs font-medium text-govuk-dark-grey py-1.5 pr-4">
                          Field
                        </th>
                        <th className="text-left text-xs font-medium text-govuk-dark-grey py-1.5">
                          Submitted
                        </th>
                      </tr>
                    </thead>
                    <tbody>
                      {rows.map((row, i) => (
                        ...
                        >
                          <td className="text-govuk-dark-grey py-1.5 pr-4 align-top">
                            {row.label}
                          </td>
                          <td className="text-govuk-black font-medium py-1.5">
                            {row.value}
                          </td>
                        </tr>
                      ))}
                  ...
                  <p className="text-sm text-govuk-black whitespace-pre-line">
                    {completion}
                  </p>
                )}
                {!disabled && (
        ...
        {!isCompleted && isDateReminder && (
          <>
            <div className="flex flex-wrap gap-2 mt-4">
              ...
              <button
                onClick={() =>
                  onComplete?.(task.id, `Dismissed: ${task.description}`)
                }
                disabled={disabled}
...
/** Summary card shown when all tasks are completed */
export function TaskSummaryCard({
  ...
  const allDone = tasks.every((t) => completions[t.id]);
  if (!allDone || submitted) return null;

  return (
    <div
      className="my-3 rounded-2xl bg-white border border-green-200 shadow-sm"
      style={{ boxShadow: "0 2px 8px rgba(0,112,60,0.08)" }}
    >
      <div className="px-5 py-5">
        <div className="flex items-center gap-2.5 mb-3">
        ...
        <p className="text-sm text-govuk-dark-grey mb-4">
          Review your answers above, then submit to continue.
        </p>
        <button
          onClick={onSubmit}
          className="text-sm font-bold text-white px-6 py-2.5 rounded-full transition-colors"
