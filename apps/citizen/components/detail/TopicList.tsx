      ...
      <div className="bg-white rounded-card shadow-sm divide-y divide-gray-100">
        {topics.map((topic) => (
          <button
            key={topic.id}
            onClick={() =>
              openBottomSheet("topic-questions", {
                topic: topic.label,
          ...
          >
            <span className="flex-1 text-sm font-medium text-govuk-black">
              {topic.label}
            </span>
            <svg
