...
export function SupportSummary() {
  ...
  const eligible = ANNA_SUPPORT.filter((s) => s.status === "eligible" || s.status === "check");
  ...
  return (
    <div className="mb-5">
      <h3 className="text-base font-extrabold text-govuk-black mb-2">Your support</h3>
      <p className="text-xs text-govuk-dark-grey mb-3">Based on your data, here's what you receive and what you may be eligible for.</p>
      ...
      <div className="bg-white rounded-card shadow-sm overflow-hidden divide-y divide-gray-100">
        ...
        {receiving.map((item) => (
          <div key={item.id} className="px-4 py-3">
            <div className="flex items-center gap-2 mb-1">
              <strong className="text-sm text-govuk-black flex-1">{item.name}</strong>
              <StatusBadge status={item.status} />
            </div>
            <div className="flex items-baseline gap-1.5">
              <span className="text-lg font-bold text-govuk-green">{item.amount}</span>
              <span className="text-xs text-govuk-dark-grey">{item.frequency}</span>
            </div>
            <p className="text-xs text-govuk-dark-grey mt-0.5">{item.description}</p>
          </div>
        ))}
        ...
        {receiving.length > 0 && eligible.length > 0 && (
          <div className="px-4 py-2 bg-blue-50/50">
            <p className="text-xs font-bold text-blue-700 uppercase tracking-wide">You may also be eligible for</p>
          </div>
        )}
        ...
        {eligible.map((item) => (
          ...
          >
            ...
            <div className="flex items-baseline gap-1.5">
              <span className="text-lg font-bold text-govuk-blue">{item.amount}</span>
              <span className="text-xs text-govuk-dark-grey">{item.frequency}</span>
            </div>
            <p className="text-xs text-govuk-dark-grey mt-0.5">{item.description}</p>
