  ...
  try {
    ...
    if (output.pipelineTrace) {
      ...
      await emitter.emit("agent.selected", chatSpan, {
        ...
        serviceId,
        reason:
          output.pipelineTrace.agentUsed === "triage"
      ...
      await emitter.emit("pipeline.trace", chatSpan, {
        ...
        agentUsed: output.pipelineTrace.agentUsed,
        aiSteps: output.pipelineTrace.steps.filter(
          (s: { type: string }) => s.type === "ai",
        ).length,
        deterministicSteps: output.pipelineTrace.steps.filter(
          (s: { type: string }) => s.type === "deterministic",
        ).length,
      });
    }
